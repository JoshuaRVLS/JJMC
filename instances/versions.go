...
func (v *VersionsManager) InstallFabric(version string) error {
	...
	// Access unsafely or use getters? Manager has no Getter for WorkDir.
	// We might need to expose GetWorkDir in manager.
	// But in process.go we have usage of m.workDir.
	// Let's add GetWorkDir to manager first or use local variable if we pass it?
	// No, we should assume manager is configured.
	// Is workDir exported or not? It's `workDir` - unexported.
	// I need to update `manager/manager.go` to expose it or method `GetWorkDir`.
	// For now, let's assume I fix manager.
